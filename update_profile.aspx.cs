...
public partial class update_profile : System.Web.UI.Page
{
    ...
    protected void btn_update_Click(object sender, EventArgs e)
    {
        using (MySqlConnection con = new MySqlConnection(constring))
        {
            con.Open();
            MySqlCommand cmd = new MySqlCommand();
            //cmd.CommandText = "UPDATE tbl_customer set (c_name='jab' where (c_email='"+Session["a"]+"'))";
            cmd.CommandText = "UPDATE tbl_customer SET c_name='" + txt_c_name.Text + "' , c_phone=" + txt_c_phone.Text + " , c_pin=" + txt_c_pin.Text + " , city='" + ddl_c_city.SelectedItem + "' where c_email='" + Session["a"] + "'";
            cmd.Connection = con;
            cmd.ExecuteNonQuery();
    ...
    protected void btn_delete_Click(object sender, EventArgs e)
    {
        using (MySqlConnection con = new MySqlConnection(constring))
        {
            ...
            cmd.CommandText = "delete from tbl_property where p_owner='" + Session["a"] + "'";
            cmd.Connection = con;
            cmd.ExecuteNonQuery();
            Response.Redirect("homepage.aspx");
            con.Close();
