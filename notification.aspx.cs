...
public partial class notification : System.Web.UI.Page
{
    ...
    private void commentct()
    {
        ...
        {
            con.Open();
            MySqlCommand cmd = new MySqlCommand("select * from tbl_enquiry_temp where e_owner='" + Session["a"] + "' AND e_status NOT IN ('Approved')", con);
    ...
    protected void dlc_ItemCommand(object source, DataListCommandEventArgs e)
    {
        if (e.CommandName == "delete")
        {
            ...
            string update2 = "update tbl_enquiry set e_status='Rejected' where enq_id=" + enq_id + "";
        ...
        {
            MySqlConnection con = new MySqlConnection(constring);
            string p_id = ((Label)e.Item.FindControl("lbl_prod_id")).Text;
            string enq_id = ((Label)e.Item.FindControl("lbl_enq_id")).Text;
            string update1 = "update tbl_property set p_status='Approved' where p_id=" + p_id + "";
            string update2 = "update tbl_enquiry set e_status='Approved' where enq_id=" + enq_id + "";
            string update3 = "update tbl_enquiry_temp set e_status='Approved' where enq_id=" + enq_id + "";
            MySqlCommand cmd1 = new MySqlCommand(update1, con);
            MySqlCommand cmd2 = new MySqlCommand(update2, con);
            MySqlCommand cmd3 = new MySqlCommand(update3, con);
            con.Open();
            cmd1.ExecuteNonQuery();
            cmd2.ExecuteNonQuery();
